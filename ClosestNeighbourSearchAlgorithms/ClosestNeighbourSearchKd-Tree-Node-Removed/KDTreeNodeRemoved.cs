...
namespace ClosestNeighbourSearchAlgorithms
{
    ...
    /// </summary>
    [Serializable]
    public class KDTreeNodeRemoved<TDimension> where TDimension : IComparable<TDimension>
    {
        /// <summary>
        ...
        #region Not Used
        /// <summary>
        /// Gets a <see cref="BinaryTreeNavigatorNodeRemoved{TPoint}"/> that allows for manual tree navigation,
        /// </summary>
        public BinaryTreeNavigatorNodeRemoved<TDimension[]> NavigatorNodeRemoved => new BinaryTreeNavigatorNodeRemoved<TDimension[]>(this.InternalPointArray);
        #endregion
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="KDTreeNodeRemoved{TDimension}"/> class.
        /// </summary>
        /// <param name="dimensions">The number of dimensions in the data set.</param>
        /// <param name="points">The points to be constructed into a <see cref="KDTreeNodeRemoved{TDimension}"/></param>
        ...
        /// <summary>
        /// Finds the nearest neighbors in the <see cref="KDTreeNodeRemoved{TDimension}"/> of the given <paramref name="point"/>.
        /// </summary>
        ...
        {
            var nearestNeighborList = new BoundedPriorityListNodeRemoved<int, double>(neighbors, true);
            var rect = HyperRectNodeRemoved<TDimension>.Infinite(this.Dimensions, this.MaxValue, this.MinValue);
        ...
        {
            var nearestNeighbors = new BoundedPriorityListNodeRemoved<int, double>(neighboors == -1 ? this.Count : neighboors, false);
            var rect = HyperRectNodeRemoved<TDimension>.Infinite(this.Dimensions, this.MaxValue, this.MinValue);
            ...
            IReadOnlyCollection<TDimension[]> points)
        {
            ...
            var sortedPoints = points.OrderBy(z => z[dim]).ToArray();
            ...
            if (leftPoints.Length <= 1)
            {
                if (leftPoints.Length == 1)
                {
                    this.InternalPointArray[BinaryTreeNavigationNodeRemoved.LeftChildIndex(index)] = leftPoints[0];
                }
            }
            else
            {
                this.GenerateTree(BinaryTreeNavigationNodeRemoved.LeftChildIndex(index), nextDim, leftPoints);
            }
            ...
            if (rightPoints.Length <= 1)
            {
                if (rightPoints.Length == 1)
                {
                    this.InternalPointArray[BinaryTreeNavigationNodeRemoved.RightChildIndex(index)] = rightPoints[0];
                }
            }
            else
            {
                this.GenerateTree(BinaryTreeNavigationNodeRemoved.RightChildIndex(index), nextDim, rightPoints);
            }
        }
        ...
        /// <param name="rectNodeRemoved">The <see cref="HyperRectNodeRemoved{T}"/> containing the possible nearest neighbors.</param>
        /// <param name="dimension">The current splitting dimension for this recursion branch.</param>
        /// <param name="nearestNeighbors">The <see cref="BoundedPriorityListNodeRemoved{TElement,TPriority}"/> containing the nearest neighbors already discovered.</param>
        ...
        private void SearchForNearestNeighbors(
            int nodeIndex,
            TDimension[] target,
            HyperRectNodeRemoved<TDimension> rectNodeRemoved,
            int dimension,
            BoundedPriorityListNodeRemoved<int, double> nearestNeighbors,
            double maxSearchRadiusSquared)
        {
            ...
            var leftRect = rectNodeRemoved.Clone();
            leftRect.MaxPoint[dim] = this.InternalPointArray[nodeIndex][dim];

            var rightRect = rectNodeRemoved.Clone();
            ...
            var nearerNode = compare <= 0 ? BinaryTreeNavigationNodeRemoved.LeftChildIndex(nodeIndex) : BinaryTreeNavigationNodeRemoved.RightChildIndex(nodeIndex);
            var furtherNode = compare <= 0 ? BinaryTreeNavigationNodeRemoved.RightChildIndex(nodeIndex) : BinaryTreeNavigationNodeRemoved.LeftChildIndex(nodeIndex);
